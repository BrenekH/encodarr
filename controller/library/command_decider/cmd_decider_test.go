...
func TestGenFFmpegCmd(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			params:   jobParameters{Encode: true, Codec: "hevc_vaapi", Stereo: false, HWDevice: "/dev/dri/renderD128"},
	...
	for _, tt := range tests {
		...
		t.Run(testname, func(t *testing.T) {
			ans := genFFmpegCmd(tt.params.Stereo, tt.params.Encode, tt.params.Codec, tt.params.HWDevice)
